function [bq_estCurr,ch_estCurrT]=separa(y_GI_Esti,Nesti,dn_esti,Lbq,Lch,P,Ng,Nsym,Nfft,bq_CO,ch,dispTemp,PAbasicAmp)
...
if(dispTemp==1)
    figure;
    subplot(321);   
    plot(bq_CO,'ro');   hold on;    plot(bq_estCurr,'b+');
    legend('bq_CO','bq_estCurr');

    subplot(322);   
    plot(real(ch),imag(ch),'ro');   hold on;    plot(real(ch_estCurr),imag(ch_estCurr),'b+');
    legend('ch','ch_estCurr');

    subplot(323);    % 某帧内各OFDM符号迭代时的nmse信息
    plot(nmse_bqFrame,'-o');  title('nmse_bqFrame');    grid on;

    subplot(324);
    plot(mmse_chFrame,'-*');  title('mmse_chFrame');    grid on;

    subplot(325);
    plot(OBOVec,'*');title('OBOVec');
end
...
if(dispTemp)
    figure;
        M=3;    N=4;
        for kk=1:(M*N/2)
            subplot(M,N,kk*2-1);    plot(abs(dnMem(:,kk)),abs(xnMem(:,kk)),'*');  title('AM-AM');     hold on;
            ang = angle(dnMem(:,kk)) - angle(xnMem(:,kk));
            subplot(M,N,kk*2);    plot( abs(dnMem(:,kk)), unwrap(ang),'*');  title('AM-PM');     hold on;
        end
end       
